        ...
        while (position < queue.size()) {
            ...
            try {
                for (int i = position; i < queue.size() && i < position + NUM_PROCESS; i++) {
                    ...
                    values.put(BaseColumns._ID, song.getSongId());
                    values.put(AudioColumns.TITLE, song.getName());
                    values.put(AudioColumns.TRACK, song.getTrack_number());
                    values.put(AudioColumns.YEAR, song.getDateAdded());
                    values.put(AudioColumns.DURATION, song.getDurationLong());
                    values.put(AudioColumns.DATA, song.getPath());
                    values.put(AudioColumns.DATE_MODIFIED, song.getDate_modified());
                    values.put(AudioColumns.ALBUM_ID, song.getAlbumId());
                    values.put(AudioColumns.ALBUM, song.getAlbumName());
                    values.put(AudioColumns.ARTIST_ID, -1);
